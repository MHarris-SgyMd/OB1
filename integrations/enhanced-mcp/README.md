...
- **The tier and date filters apply to the rows, after the database ranks them (SMD-1986).** The three search tools hand the database the caller's `metadata_filter` alone: both functions they call read that argument as a metadata containment (`metadata @> filter` — this fork's `match_thoughts`, `db/migrations/014`, and the enhanced-thoughts sidecar's `search_thoughts_text`), so the `exclude_restricted`, `start_date` and `end_date` keys the tools used to fold into it matched no thought, and every search on this fork answered "No matches found." The restricted tier is dropped from the results by the `sensitivity_tier` column — in semantic mode through one lookup of the returned ids, since `match_thoughts` returns no such column — and the date bounds are compared against each row's `created_at`. Two consequences. In **semantic mode**, `match_thoughts` returns the top-N by cosine similarity; with a date bound the tool over-fetches 3× the requested limit (capped at 500, the count the function clamps to) and filters, so on a brain with very dense recent activity and a narrow old window it can miss matches ranked below the over-fetch. In **text mode**, `search_thoughts_text` ranks and pages before the tier and date filters apply: a page can hold fewer than `limit` rows, `pagination.total` counts rows the filters hide, and `has_more` says whether the cursor has another page (a page the filters emptied says "No matches on this page; more follow."). For an exact count under a date window use `count_thoughts`, which filters in SQL; a follow-up gives `search_thoughts_text` tier and date arguments of its own.
