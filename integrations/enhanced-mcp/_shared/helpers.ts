...
export async function fetchWithTimeout(
  ...
  const defaultMs = Number(process.env.FETCH_TIMEOUT_MS ?? 60_000);
...
export function embeddingModelUsed(): string {
  if (process.env.OPENROUTER_API_KEY) return process.env.OPENROUTER_EMBEDDING_MODEL ?? "openai/text-embedding-3-small";
...
function getLlmCallCap(): number {
  const raw = process.env.ENHANCED_MCP_MAX_CALLS;
