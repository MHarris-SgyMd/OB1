...
| `/writeback` | POST | Save compact operational memory after work finishes — each memory's thought through the 3-argument `upsert_thought`, its vector and model label in the same call (FORK.md change 68; the vector is `openai/text-embedding-3-small`'s, so the brain must be at that model and width — 1536, not the fork's 1024 default — or every write-back fails whole) |
