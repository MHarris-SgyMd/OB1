...
- A brain built as `SETUP.md` describes, with two conditions the gateway adds. **Its embedding width:** the gateway embeds through OpenRouter with `openai/text-embedding-3-small` (1536 wide, fixed in its code), so build the brain at `OB1_EMBEDDING_MODEL=openai/text-embedding-3-small` and `OB1_EMBEDDING_DIM=1536` — `SETUP.md`'s local default (1024) makes every capture and semantic search answer 500, since `upsert_thought` refuses another width (the gateway's README says the same). **Its columns:** apply `schemas/enhanced-thoughts/schema.sql` after the migrations — every read selects seven of the columns it adds (`type`, `source_type`, `importance`, `quality_score`, `sensitivity_tier`, `status`, `status_updated_at`), and without it browse, text search and stats answer 500 `column "type" does not exist`; the gateway's README lists `schemas/workflow-status/migration.sql` beside it, which adds the same two status columns idempotently and backfills `status = 'new'`.
- The gateway running against that brain on a host this dev host can reach — from a checkout of this repository, on the brain host, after `cd extensions && bun install` (the vendored servers resolve their packages from that install through `NODE_PATH`; `--no-install` makes a forgotten `NODE_PATH` or a missing install fail naming `hono`, where Bun would otherwise fetch an unpinned copy from npm and start anyway):
...
- **HTTP 500**: the body says which — `OPENROUTER_API_KEY is not configured`, `OpenRouter embeddings failed: 401 …` (the key is wrong), `expected 1024 dimensions, not 1536` (the brain's width), `column "type" does not exist` (the schema above is not applied). The gateway prints nothing for these; the body is the whole diagnosis. Gateway-side, not this dev host's.
