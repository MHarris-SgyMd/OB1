...
The Obsidian vault import recipe retires — the Markdown adapter is the fork's import, one file one thought with the file kept byte for byte, and what the recipe had that the adapter has not is named, dropped or filed on the adapter's and the connector's tickets, nothing ported (SMD-2137)
...
typecheck; `test-live.ts` 728 of 728 on the branch.
