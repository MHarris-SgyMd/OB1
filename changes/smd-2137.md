...
(`local-ollama-embeddings`, the sibling retirement); SMD-2221 (filed in pass 3:
an adapter flag without `--source` runs every source, and a bare `--allow`
name is not resolved as `--markdown`'s is — zero written, exit 0).
