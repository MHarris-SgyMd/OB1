...
Check 20 in `scripts/check-fork-consistency.ts` runs twenty-eight probes and
...
| 3 | `$q$DELETE FROM $q$ \|\| t \|\| ' WHERE …'` was a hit — the delete was read on the blanked path, its `WHERE` in a piece the blanking hid; the blanker's tag grammar had no probe; `TRUNCATE Übersicht` missed | run-it | a `$tag$ \|\|` statement read raw; a `$q1$` probe; Unicode name classes |
