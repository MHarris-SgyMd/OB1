...
inside a string is dynamic SQL and is read raw, its `WHERE` concatenated on
or in the same template. String literals are read, as check 12 reads them,
...
Check 20 in `scripts/check-fork-consistency.ts` runs twenty-six probes and
thirty-two non-probes through the rules every run — the tree's own shapes
...
would track (check 15's listing; 84 today; a listing that finds none fails
...
the listing matching no file fails in words; check 20 not called passes a
...
| 1 | the disk walk read gitignored files, against `.gitignore`'s promise of the checker | run-it | check 15's git listing |
...
files in check 20's scope. A per-PR gate step: the checker runs on every push
over the whole tree, stricter than upstream's changed-files grep.
