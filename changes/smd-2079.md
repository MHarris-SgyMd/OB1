...
`Content-Type` and nothing else, and its seventeen callers that passed `req`
...
none — every handler's `200` and most of its errors — and the handler
returned their answers untouched (SMD-2054's record says forty-seven and
fourteen: a one-line grep's count, which missed two calls with `req` on a
line of its own; the paren-balanced count is sixty-two, seventeen and
forty-five). So under an allowlist, the configuration the
...
`429` — `RATE_LIMIT_PER_MIN` set low before the module loads and `/health`
called until it answers — carries the headers beside `Retry-After` and its
JSON content type, the set this change rebuilt, and exposes `Retry-After`; a fourth drives `POST
/ingestion-jobs/:id/execute` through the suite's fetch stub, which now
answers smart-ingest's execute after 25 ms with the abort signal honoured, so
the proxy's timeout must be a number for the `200` to arrive. Against
