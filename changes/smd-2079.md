...
`route`'s `try` — the preflight branch, the auth and rate-limit steps,
`new URL(req.url)` and the path regexes stand before it, so a failure there
was an unhandled rejection, Bun's bare `500` with no headers.
...
README's Edge-Function residue); SMD-2113 (the sibling gateways read no
allowlist).
