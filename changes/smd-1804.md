...
The fork has a version — `MAJOR.MINOR.PATCH+upstream.<sha>` — that the brain reports (`schema_version`, migration 044) and preflight checks, plus `CHANGELOG.md` and `changes/` fragments assembled into FORK.md at release (SMD-1804, migration 044).
...
release step assembles fragments into FORK.md sections and `CHANGELOG.md`
(Keep a Changelog 1.1.0) in one commit, assigning the numbers once. Sections 1–100
stay; the transition accepts both, so in-flight branches need no rework.
