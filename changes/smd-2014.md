...
not the session's. Guarding against a second live thought when the state file
is LOST and the model, being non-deterministic, writes different text on the
re-post (review pass 2): it needs a state-file loss and a server that dedups by
content, the derived-mode path shares the first half of that edge, and the
model text is persisted through the ordinary retries the state does survive.
Rejecting a non-finite metadata number (`1e400` → `Infinity`): a client's
`JSON.stringify` turns it into `null` before it is sent, so only a hand-crafted
raw body delivers it and it stores as a benign `null`. Skipping the model for
an episode whose assistant excerpt is empty: an episode with prompts and no
assistant text is rare, and the model can still shape the derived text — a
wasted call in the detached child, not a defect. Trimming a `/chat/completions`
