...
**Review passes.** Three: a cold read plus six mutants on a scratch copy (five
bit; the sixth crashed the suite), then `/code-review` at high effort twice.
Pass 3's findings sat in pass 2's fixes — the stop signal; SMD-1973 and
SMD-1974 hold the two without a local reproduction.
...
SMD-1974: chunk.ts cannot window a whitespace-free run, so a pasted blob is
one window however long — a chunk.ts fix, since it reaches embeddings too.
