...
rules − 24 marker − 256 floor) / 3`), never above
...
entry derives 456 uncapped through the resolver; unknown model → default; the
...
**Review passes.** One: an independent cold read of the diff and six
drop-the-mechanism mutants on a scratch copy (five bit with a counted failure;
the sixth crashed the suite). Stop signal at one — every finding was in the
new code and none in a fix.
...
**Follow-ups.** SMD-1731 (lineage: the `parts` record's home). SMD-1960:
stream the extraction call and abort a runaway on its third repeated item,
where today it runs to the budget (~30 s) first. SMD-1961: a labelled
long-document corpus — the header and the retry's thinner answer were decided
on three planted documents with no precision measured. SMD-1962: Bun's 300 s
idle cut is disabled in the three worker diallers only; preflight's probes and
the evals' own diallers still run under it — one `providerFetch` and a check.
Operational: the dogfood brain needs `--switch-key` for p2 and a
`--retry-failed` pass.
