...
**Review passes.** Two: a cold read plus six mutants on a scratch copy (five
bit; the sixth crashed the suite), then `/code-review` at high effort. No
pass-2 finding was in a pass-1 fix; the one without a local reproduction
(hosted providers rejecting the new request fields) is SMD-1973.
...
SMD-1973: the budget and the retry are unmeasured on any hosted provider; an
endpoint that rejects `max_tokens` or `frequency_penalty` stops the pass as
fatal, and the fix wants a measured endpoint before a downgrade or a knob.
