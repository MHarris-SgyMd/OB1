...
restricted) before it drops a row — the same lookup supplies the `type` and
`source_type` the semantic results carry, which `match_thoughts` returns no
more than the tier. The date bounds are `dateWindow` and `withinDates`,
...
fork note says so. `GET /recent`, the one content route with no tier filter
at all, takes `exclude_restricted` (default `true`) as `/thoughts`, `/count`
and `/thought/:id` do.
...
must drop the twin. Eleven assertions: semantic mode and text mode find the
capture and not the twin, text mode with `total: 2`, `count: 1`; a semantic
result carries the row's `type` and `source_type`;
...
through enhanced-mcp's copy alone. On the route as it was, six of the first
seven failed: the leak (two rows, the twin among them), the empty text page,
the bounds, the refusals and the page. 335 assertions on a throwaway
Postgres, CI's step under `TZ=America/Chicago`. Found on the way: the suite's `send` silenced
...
| 1 | `?? rows.length` behind `total_count` could never fire (the count is non-null on every returned row); a page past the last hit answers `total: 0`, unsaid and undriven | cold-read (second reviewer) | removed; the README says it; page 3 is driven |
...
**Not taken.** A shared module for the two servers' date helpers: the
