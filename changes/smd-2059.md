...
removes; a report of orphans is a follow-up). A `created_at` on the sync's
...
the parts for every ticket; a report of parts whose section is gone;
SMD-1949's enum decision reads the new `observation` count.
