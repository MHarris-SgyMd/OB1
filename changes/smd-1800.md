...
default one; its findings were inside pass 1's fixes — the stop signal.
...
| 2 | pass 1's `hold()` asserted `stack.length === 4`, the regex's cardinality typed twice; the record still said 757 after pass 1 added an assertion, and "before this merges" after the PUT was applied | cold-read | `STACK` once, the regex and the count from it; 758, the PUT dated |
