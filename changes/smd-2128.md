...
rest-api does — a UUID or digits, as a string — where `parseInt` 404'd every
UUID beginning with a letter and fetched a truncated integer for one beginning
with a digit: the per-item link this change lights lands (review pass 1; the
dashboard's `Thought.id: number` typing stays SMD-2152's). The two READMEs
describe the form, the columns, the retype and its lock, the re-grant;
...
columns never written (two), no actor (five), a revision without `supersedes`,
the item's result written as status alone (four), a fingerprint match naming
no thought, the retype disabled with the function on `bigint` (six, every item
arm — as on main), the result column not filled from SMD-2110's key; pass 1's
`/execute` stepping over a skip, the inline skip's status alone, an
`embedding` key after the spread, the raw view error; pass 2's failed skip
flipped, no `failed_count`, a revision of text already there reported revised,
the DETAIL dropped; pass 3's handler on one SQLSTATE again.
