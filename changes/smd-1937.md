...
The entity-gate spike, measured on 201 blind-graded dogfood mentions the deterministic gate cannot judge: Verdict v1.4 behind the typed-decision tier does not beat the deterministic shape gate by the pre-registered margin (+10.1 points of balanced accuracy, 95% interval −1.0 to +21.0), under either rubric the grades were written to, and types worse than the extractor; a diagnosis shows it scores the note against its labels and never reads the name, a task that needs knowledge of names its benchmark does not ask for; the deterministic gate ships first, and nothing enters the extraction path (SMD-1937).

## FORK

A typed-decision gate after the entity extractor, measured on the cases a regex cannot judge — no margin over the deterministic gate under any rubric, and the model never reads the name; nothing enters the extraction path (SMD-1937)
...
- **Also reported:** typing, SMD-1925's threshold table, a permutation test, the
  cost of whole thoughts.
- **Flags:**
...
- **Rubric v2**, on the maintainer's call, grades the entity (up to three
...
- **Why it fails.** `--diagnose`:
  - It never reads the name: the entity, `021` and `banana` get one answer on
    56 of 60 windows.
  ...
  - A JevBench answer is in its text; an entity's type rarely is, so the task
    needs knowledge of names.
  - A trap: examples taken from graded names scored 48%, by string matching.
- **What it settles.**
  ...
  - Code artifacts are entities, by the maintainer's decision. So about 28% of
    ...
    splits, and SMD-1935's gate must not reject code artifacts.

**Review passes.** Each pass used independent readers at once: definitions,
run-it with mutants, and a cold read.
