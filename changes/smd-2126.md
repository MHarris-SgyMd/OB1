...
4. Check 24: in every code file (.ts/.tsx/.mts/.cts/.js/.jsx/.mjs/.cjs, .py,
   ...
   Python and shell — for shell at a word's start alone, since `$#` and
   `${#a[@]}` are not comments (a docstring is a string and is read: a file
   ...
   remaining size. Twenty-eight probes and two blanker probes run inside the check.
