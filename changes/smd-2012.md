...
**Held.** `test-session-capture.mjs` (351 assertions, in CI's
...
refused. Mutants over the passes — no line, SessionEnd
...
unseen, zero accepted, the flags read after the transcript — each fail at
least one assertion.
