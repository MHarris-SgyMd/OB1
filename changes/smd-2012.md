...
— `--print-hook codex --event PreCompact` is refused with the Stop alternative
named. `--event` is held to the table by one reader for `--print-hook` and
`--dry-run`: a misspelt event (`precompact`) would have installed a hook that
...
`hookJson` refusing an unknown event by name; the obsolete reason's words; a
closing longer than the cap. Mutants over both passes — no line, SessionEnd
...
negative bound back — each fail at least one assertion.
...
summary that is right about its own state. Compactions with no turn between
them post again at each for the same reason. A structured checkpoint field on
