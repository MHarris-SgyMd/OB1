...
the line. The Stop interval stays Stop's alone: a compaction is rare and an
...
**Held.** `test-session-capture.mjs` (325 assertions, in CI's
...
the pair; `--dry-run --event`'s preview and its refusal. Seventeen mutants — no
line, SessionEnd alone as the default, each refusal dropped, the timeout
dropped, the trigger unvalidated at either site, the checkpoint never set, Stop
read as compacted, the time from the clock, the log silent, the closing lines
clipped, the TypeError back, the dry-run preview dropped — each fail at least
one assertion.
...
summary that is right about its own state. Two compactions with different
triggers and no other change post twice for the same reason.
