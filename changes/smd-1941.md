...
server, a `capture_thought` call that names `source:leak` under
`OB1_EGRESS_ALLOW=source:leak` is still refused at zero requests while the row
RECORDS `source:leak` — the claim is stored, it does not open the gate.
`bun server-portable/test-preflight.ts`, 175 assertions: the `source:` warning
says the term does not gate a capture and points at `actor:`.
