...
- The deploy-stack CI job:
  ...
  - adds a canary step:
    ...
    - a host `http.server` on the port, refused up front;
    - a probe thought whose text holds literals, found through the strip;
    - one `up` with the stub answering random vectors, which must fail its smoke;
    - an unlabelled volume by the canary's name holding a table, refused, with nothing left running;
  ...
  - extends the runner's provider stub to answer `POST /v1/embeddings`. It embeds as a bag of words hashed into 1024 dimensions, the model's query instruction left off, or as random vectors while a flag file exists. The probe thoughts are seeded with its vectors for their text.
...
- Pass 3's probe, on the live canary (read-only but for its query log):
  - The 30 newest candidates scored 78.5–94.3% against their own text.
  - 20 came back as Result 1. The other 10 lost to a sibling session summary at 80–88%.
  - The five-candidate rule passed from all 26 starting points, with at most two misses in a row.
- tier.sh under a process-group SIGINT, mid-refresh and very early, under docker and podman: exit 130 each time, no container left, and the target never migrated.
