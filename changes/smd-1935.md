...
The entity graph refuses a number or a type word the extractor names and retypes an identifier-shaped person or place, at the writer and over the rows before it; the people facet takes the same rule (SMD-1935)
...
code artifacts, which the maintainer decided are entities, typed `place` — for
1 junk mention. So a shape corrects the type. A ticket id becomes a project
...
twin is held to it by test-schema [52]: 60 probes asked of both engines, both
normalisers compared, and the SQL body checked to spell every pattern, the
trim and every vocabulary word `entity-gate.ts` does, no more, with the two
handle shapes read for a place only. Every class is spelled out, whitespace
included (`[^ \t\n\r\f\v]`, never `\S`), since Postgres reads `\w` and `\s` by
locale; the normaliser may still part on non-ASCII whitespace (U+2028,
U+0085), where the numeric rule can refuse in one engine and not the other —
only the people facet reads the JavaScript side.

**Measured after.** 056 applied to a fresh copy of the dogfood brain, at
this commit: 116 refused (165 mentions, 153 edges), 8 moved, 4 merged
...
| 3 | a `--grant` role lacks UPDATE on `thought_entities` and `ob1_entity_edges`, which 053's writer upserts (before this branch) | run-it | filed SMD-2216 |
| 3 | smaller: the pass's comment, "a retype loses nothing", the re-extraction claim, a probe comment; extract-entities printed a gate's zeros against a brain without 056 | cold read | fixed, each |

**Not taken.**
- *A numeric name a structured source states.* A Linear label `2024` stays,
  so the ticket's `name ~ '^[0-9]+$'` can return rows after 056 — each one a
  name the source states. graph-centrality's read-side numeric rule
  (`--keep-numeric`) still filters them, and a brain from before 056.
- *Binding "migration 021" to a `migration:021` reference.* No such type; the
  ticket allows dropping it.
- *Re-extracting the affected thoughts.* The gate reads only a name and a
  type, so the pass applies it to the stored rows without the model. It
  judges each entity on its first-seen spelling, where a re-extraction judges
  each answer's, and it leaves curated entities, so the two can differ on
  those.
...
- *Keeping a structured row in a merge.* `merge_entities` keeps the target's
  row, and the pass follows it; the pass skips any entity a structured pass
  names, so no structured row is merged.
