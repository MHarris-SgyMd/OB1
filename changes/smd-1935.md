...
- **`apply_entity_type_gate()`** applies the rule to the rows written before
  it, each entity judged on its name (its first-seen spelling) and none a
  structured pass names. A refused entity's edges, mentions and row are
  ...
  moves. Its `merged_from` goes either way: a human's merge within the old
  type is no decision about the new one. The file runs it once and reports
  `{refused, dropped mentions and edges, moved, merged}` as a NOTICE; it is
...
- *Re-extracting the affected thoughts.* The gate reads only a name and a
  type, so the pass does over the stored rows what a re-extraction's write
  would, without the model.
...
- *Narrower rules for a general brain.* The shapes were measured on an
  ...
  A retype loses nothing; the refusal does.
