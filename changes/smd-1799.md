...
— the nine files that handed `Deno.serve` an inline function bind it as
`const handler = (req: Request) => …` and export that. Bun serves the entry
...
`Deno.<member>` is refused with the shape to port it to. 21 READMEs and
`primitives/deploy-edge-function` say "Bun-native" where they named the
polyfill; `compat/supabase-sql/README.md` §3 shows the tail.
...
SMD-2001's home is the default export's `fetch`.
