...
function bind it as `const handler = (req: Request) => …` and export that. Bun serves the entry
module's default export when it has `fetch` (measured on 1.4.0: `port` in the
...
Dockerfile runs `deno serve --port 8000 index.ts` (which serves that export;
`deno run` would load the module and exit) and `k8s/openbrain.yml` drops its
`PORT`; the `denoland/deno` image stays SMD-1800's. `compat/deno-on-bun.ts` is
...
`Listening on` line to parse, Bun's own `Started development server:` line
unread.
...
still imports no `jsr:`/`npm:`/URL specifier, transitively. Twelve `Deno`
probes and six specifier probes hold the rule. `scripts/migrate-to-sql-shim.ts`
...
**Review.** Pass 1, two reviewers, cold-read: the checker's blanker read a
template literal whole, `${Deno.env.get("X")}` included, and a quote inside a
regex literal opened a "string" that hid the code after it (the session-capture
hook's redaction regex was the tree's witness) — `blanked` keeps `${…}`
expressions as code and blanks a regex literal now, seven probes hold it; check
11 read .ts/.js/.mjs only — every code extension now, with a probe; a missing
exception file failed twice; nine READMEs still promised the polyfill's
`Listening on` line (Bun's `Started development server:` line, or `curl`); the
primitives note still had two extensions deploying as Edge Functions; an empty
`PORT` was a random port (`||`, above); the template and shared-MCP samples
taught `Deno.env.get` and `Deno.serve`; the compat README, the codemod's
docblock, FORK.md's file rows and a CI comment named the polyfill as current.
