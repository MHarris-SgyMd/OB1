...
One renderer and one merge rule for a Linear ticket, whichever tool writes it — the dump carries the issue, the pipeline honours the source's clock and, where that clock is silent, the brain's (SMD-1958)
...
`unchanged`, one with no build instant writes; the
