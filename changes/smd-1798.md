...
with one import change each, two where the polyfill is added. One call site
...
Moving enhanced-mcp onto `MCP_ACCESS_KEYS`: change 67's decision, not this
...
dependency (an npm lockfile; npm is not in this toolchain; its code is on the shim).
...
first start (unpinned, with npm egress) — every SDK-importing recipe since
change 74; a per-directory install. SMD-1992: the servers that build their client per
request accept an `https://` URL at start and answer a bare 500 to every
request; agent-memory-api answers a bare 500 when its provider refuses the
key. SMD-1986: enhanced-mcp's three search tools send
