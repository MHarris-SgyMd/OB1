...
**Review passes.** A cold read and a runner each pass, independently, beside
the drives; the second pass's top findings were against the first's additions
— the loop's stop signal — and it ran twenty mutants over the mechanisms.
...
change 74; a per-directory install. SMD-1992: a per-request client accepts an
`https://` URL at start and answers bare 500s; agent-memory-api's provider
failure is a bare 500 too. SMD-1986: enhanced-mcp's three search tools send
