...
| 3 | Pass 2 compared with the definition [8e]'s own load read, which was [8d]'s rebuild, so a helper that rebuilt wrongly every time compared equal; the first definition read, before [8c], is the one compared | mutant | (this pass) |
| 3 | Two drop-and-rebuild paths had grown (the helper, and [8e]'s own variable, null branch and return value); one helper around a body, called twice in [8e] | cold read | (this pass) |
