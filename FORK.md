...
Eighty-three numbered changes on top of the pin. Seven fix defects found in an
...
sections. Changes **18–83 are the numbered `###` sections** further down, which is
...
<17 pin sites, 3 lockfiles>      # change 82 (@hono/mcp 0.1.1 → 0.1.5: the transport lets go of each POST it has answered; the after sample's sweep closes the transports it drops)
<19 pin sites, 3 lockfiles, 15 servers, 20 SDK importers> # change 83 (SDK 1.30.0, @hono/mcp 0.3.2, hono 4.13.8, zod 4.6.5 together; the Accept patches removed; an @ts-types pragma on every SDK import so Deno types it)
...
resolve and the build (change 83 removed the patch: at `@hono/mcp` 0.3.x the
...
SMD-1616; the probe kept the header for those two until change 83 moved the
...
says the bound; SMD-1607 held the library fix, and change 82 moves the pin to
...
### 82. The `@hono/mcp` pin moves from 0.1.1 to 0.1.5 — the transport lets go of each POST it has answered, so a transport kept for a session (the cost recipe's after sample) no longer holds one Request and one Context per tool call until the session is swept (SMD-1607)
...
83 made that move). So the pin moves to 0.1.5 at every site that names it —
...
covered change 82 with this one, and its findings there are recorded there).
...
either-token check is worth the SDK and hono moves it needs — change 83
...
### 83. The MCP stack moves together — SDK 1.24.3 → 1.30.0, `@hono/mcp` 0.1.5 → 0.3.2, hono 4.9.2 → 4.13.8, zod 4.1.13 → 4.6.5: a second `connect()` on one server now throws, the transport takes whatever Accept a client sends and the fifteen Accept patches are gone, and every SDK import carries the `@ts-types` pragma Deno needs to type it (SMD-1643, SMD-1616)

**Why now.** Change 82 found that the defect it fixed had been fixed in the
...
every Request (change 82's assertion holds at 200/200). The build cost on
...
**Verified.** `bun test-auth.ts` **809/809** (775 at change 82: thirteen
...
changes 82 and 83 together; ten findings, two of them 82's and recorded
