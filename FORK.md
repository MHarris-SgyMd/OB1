...
`COALESCE(thoughts.x, EXCLUDED.x)`, add if empty, never change — because
`update_thought` then had no way to set provenance after the fact. It never
asked whether the pointer it filled closed a loop: change 54's cycle walk
...
about 145 such captures a second at 1,024 dimensions whatever the worker
count. The lock existed only for the fill. A fresh row cannot close a loop;
only a re-capture can. But whether a capture is a first capture or a
re-capture is known only once the fingerprint lock is held, and 033's order
puts the supersession lock before that one — so every capture naming
`supersedes` took it.
...
existing thought since it landed — walked, audited, one function — so the
fill's reason is gone, and a rule with
one owner beats the same rule in two bodies. A capture of text that is already
there is a dedup: it merges metadata and, with a vector, moves the vector,
label and windows by 021's and 022's rules; it does not decide what the
...
The 2-argument body is carried verbatim from
...
three of messages, one of tests — and one behaviour stated.** Three
reviewers, one each on the SQL, the callers and
...
behaviour both the SQL and the callers reviewer found is the ghost-UUID
re-capture, stated below under "Closed, and not" and held by [35] with a first
capture's FK still refusing. The wording: the capture
...
`supersedes` shapes and `derived_from` alone through the server. Stated as
well: `existed` is "a row held this
...
**A fourth pass, at the user's call, at another altitude: hostile inputs, the
operator who skipped a week, and a cold read.** Pass 3's two fixes held under
probes (the anchor is unique and ordered; every id form the shape check admits
compares lower-case). Hostile inputs at the tool boundary found four things,
two of them older than this change: a first capture naming a thought that does
not exist returned Postgres's foreign-key text after the embedding was paid
(since 025) — the reply now says it as `update_thought` does; a `derived_from`
element that is no id paid both model calls before the write refused it (since
032) — its shape is pre-checked beside `supersedes`'; an empty `derived_from`
fired the note though it names nothing; and the note's advice could name an
edit `update_thought` refuses for a loop as well as for a missing thought — the
advice now carries the condition, and [7] drives both refusals. The skipped
week — a brain at 032 with filled pointers, a loop, an accepted proposal, a
legacy row and windows, upgraded through 033, 034 and 035 in one run, then
`--reapply`, then 025, 033 and 034 re-applied by hand and `--reapply` again —
ended byte-identical to a fresh install in every function body, ACL and
COMMENT, with preflight naming each hand re-apply on the way; two hundred
captures, two hundred edits and fifty reviews with a backfill mid-way gave no
deadlock and no loop of any length. It also read three sentences too strongly:
"nothing written" on a dedup (the merge moves `updated_at`), "when `supersedes`
is named" (a JSON-null clear takes no lock), and a loop query that returned
each loop twice. The cold read caught a renumber miss hidden by a line wrap in
change 63's note, a duplicated statement, a contradiction between two
paragraphs about what a dedup refuses, and the seams where four passes spliced
sentences past the wrap.
...
(raw SQL) — and the ceiling with it. Changed, and stated: 025's "a re-capture
may add provenance the row did not have" is gone; a caller that later wants
to record what a captured thought supersedes calls `update_thought` with the
envelope, which the capture tool's reply names. Not this change's:
...
names no thought, or one whose chain reaches this thought, is not refused —
the FK and the walk ran only where a pointer is written, and a re-capture
writes none (a first capture's FK still refuses a missing target, now in the
tool's words rather than Postgres's) — so `existed` comes back true and no
provenance is written (the dedup's own merge runs: metadata, `updated_at`),
and `update_thought`, which the reply names with the
condition spelled out ("if that thought exists and closes no loop"), refuses
it by name (`SUPERSEDES_NOT_FOUND`, `WOULD_CYCLE`); the caller learns one step
later, not never.
...
ms at 033, 7.22 /
6.27 ms at 035. The per-call cost is the HNSW insert either way; the lock only
...
validation still refuses a malformed envelope on a dedup; SMD-1453's sequence
writes no loop and `update_thought` refuses the same pointer; a capture naming
`supersedes` holds one advisory lock inside its transaction; both COMMENTs;
the trap — 033 re-applied by hand puts the fill and the lock back and writes
the loop, 035 restores. [22], [23], [31], [33] follow the last definer ([33]'s
own trap had restored `update_thought` by re-applying 033's whole file, which
put 033's capture bodies back for every section after it; it restores both
names now). The 4-argument pass-through is read from 013's source there and
called on a real server in `test-live` [13]: PGlite aborts with a WASM
out-of-bounds on a windowed capture through that form, at 033 as at 035.
