...
### 66. delete_thought joins the writers' lock order, closing a deadlock between an accept and a delete of the superseded thought (SMD-1462)
...
**The fix (migration 035) — two changes, and running it proved both are
...
against forty deletes: the pre-035 lockless delete (009's body by hand) deadlocks,
...
`db/test-schema.ts` [35] pins 035 as the last definer of both functions and each
