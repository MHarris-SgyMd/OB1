...
three evals defined it away by racing single stores at the whole job. Unmeasured
(SMD-1707).
...
### 87. The knowledge-update slice was already reported and is the best one — and the number cannot see the failure MERIT means: the shipped read puts the stale value first on half the questions, and the resolving read, fed perfect chains, fixes that by changing what counts as relevant (SMD-1720)
...
was measured on the way to closing it. Like changes 31, 53, 55, 59, 79, 82 and 86
it ships **no runtime change**; the harness is a third arm set in
