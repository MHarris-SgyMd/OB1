...
**Migration 031: `renew_claims(work_type, worker_id, ttl_seconds)`.** Every
...
wrote them, `test-schema` [30] asserts 015 is still the last file to define the
...
**Proof.** `test-live` [8e]: a worker on a 5 s lease beats at 4.5 s; a claim
...
`--dry-run` that exits 0) and the new one holds. `test-schema` [30] owns the
