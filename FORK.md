...
### 83. A live row an HNSW walk cannot reach is the geometry, not the vacuum — `db/hnsw-graph.ts` reads the disconnected graph the suite's tied vectors build, and [4]/[11]/[15] join [7] on `match_thoughts`' exact branch (SMD-1632)
...
- **An HNSW walk can miss a live row over near-equidistant vectors**
  ...
  more connected). See change 83 (SMD-1632) for the measurements and the
  decision.
---
