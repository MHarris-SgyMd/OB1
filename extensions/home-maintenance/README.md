...
> **Two of this server's four tools fail on the fork today** — `get_upcoming_maintenance` (the shim has no `.not()`) and `search_maintenance_history` (a PostgREST embedded select the shim refuses at runtime); `add_maintenance_task` and `log_maintenance` work. SMD-1588 holds the shim fixes; FORK.md change 74's second review pass drove every tool against a real Postgres and found them.
