...
Every extension server authenticates the way the core server does, through `_shared/auth.ts` — a copy of `server-portable/auth.ts` beside the extensions, held byte-for-byte identical by the test: keys are named, scoped `read` or `write`, and stored as SHA-256 hashes in the server's `MCP_ACCESS_KEYS` environment variable, each revocable on its own. A read-scoped key is never given the tools that write, so it does not see them in `tools/list` — that is the key to put in a connector URL. The older single `MCP_ACCESS_KEY` still works, with write scope. Step 3 of [Run a Remote MCP Server](../primitives/deploy-remote-mcp/) mints one; `test-auth.ts` here asserts all seven servers behave this way — and, since FORK.md change 67, every vendored recipe and integration server too (`bun install && bun test-auth.ts`). Since change 78 it also fires three overlapping requests at each MCP server and asserts each is answered with its own id: a server built once and `connect()`ed to a fresh transport per request answers on the wrong one.
