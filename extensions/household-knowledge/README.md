...
> **Every tool of this server runs on the fork.** `extensions/test-tools.ts` drives all five against a real Postgres carrying this `schema.sql` in CI — `details` into `JSONB`, the four-column search, each filter on its own (FORK.md change 75, SMD-1588; change 74's review had found none of the five failing, and the suite holds it).
