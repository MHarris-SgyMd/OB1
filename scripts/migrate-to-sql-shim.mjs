 ...
 * shim's own refusals, spelled as regexes over the file: since change 75
...
const BLOCKERS = [
  // Resource embedding: one hop is served since change 75 (SMD-1588) — `relation (cols)`, `alias:fk_column (cols)`,
...
const KEEP = new Map([
  ...
  // Blocked by its two one-to-many embeds until change 75 served one hop; deployed as the Supabase Edge
  ...
  ["integrations/agent-memory-api/index.ts",
    "deploys as a Supabase Edge Function (its README, the deno-check job, test-auth.ts); its embeds are servable since change 75 — migrating it is a separate change"],
]);
