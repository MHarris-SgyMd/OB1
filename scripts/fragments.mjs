...
/** A `## <name>` body from a fragment (up to the next `## ` or the end); "" when the section is there and empty. */
export function fragmentSection(body, name) {
  const m = new RegExp(`(?:^|\\n)## ${name}[ \\t]*\\n([\\s\\S]*?)(?=\\n## |$)`).exec(body);
  if (!m) return null;
  const text = m[1].trim();
  return text.startsWith("## ") ? "" : text; // an empty section: the lazy capture ran into the next heading
}
