...
- **MCP servers must be remote over HTTP — a URL a client is pointed at — never a stdio process or a local server a client spawns.** Never use `claude_desktop_config.json`, `StdioServerTransport`, or a Node.js stdio server. Every server here is one HTTP process: the reference deployment is the Bun container in `SETUP.md` against any Postgres with pgvector, and every extension, recipe and integration server runs the same way, `bun <file>` (`primitives/deploy-remote-mcp/`). Clients connect through Claude Desktop's custom connectors UI (Settings → Connectors → Add custom connector → paste the HTTPS URL) or with the key as `?key=` on the URL or in an `x-brain-key` header; see `docs/01-getting-started.md` Step 7 for the pattern.
