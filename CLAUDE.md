...
- **Never modify the core `thoughts` table structure.** Adding columns is fine; altering or dropping existing ones is not. `thoughts` stays a table under the event-sourcing decision too — its row is a projection of `thought_audit`, written by the same three functions, and a view in its name was measured and declined (`docs/event-log-as-truth.md`, SMD-1997).
