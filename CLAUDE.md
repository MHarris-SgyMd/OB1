...
- **No `DROP TABLE`, `DROP DATABASE`, `TRUNCATE`, or unqualified `DELETE FROM`** in SQL files. The rule is that a SQL file must never destroy existing rows; a trigger that *refuses* one of these statements (`BEFORE TRUNCATE … RAISE`, as `db/migrations/045` does for `thought_audit`) is the rule applied, not a breach of it.
