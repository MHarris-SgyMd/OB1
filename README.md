...
| [Obsidian Vault Import](db/README.md#the-stable-tier--a-brain-rebuilt-from-the-records-smd-1806) | Retired (SMD-2137) — on this fork the import is core, the `markdown` row of `db/README.md`'s source table: `bun db/ingest-records.ts --url postgres://… --source markdown --markdown /path/to/vault --allow /path/to/vault`, then `bun db/reembed.ts --url postgres://…` with the embedding model declared as the server has it (`OB1_LLM_LOCAL=1` and the model variables, `SETUP.md`) | — |
