 ...
 */
export type PoolState = { recorded: ClaimStatus | "none"; elsewhere: boolean };
...
export function poolState(pairs: readonly { key: string; status: ClaimStatus }[], recordedKey: string | undefined): PoolState {
...
export type ThoughtRow = {
   ...
   * it sorts before it — a false fresh, said in the report).
   */
...
export type StaleGraph = {
  ...
  failed: number;
  /** Under the recorded key: succeeded, and the text moved after — no pass under this key will return to it. */
 ...
 * whether a re-read is queued, gave up, finished before the move, or was
 * never asked.
 */
export function staleGraph(rows: readonly ThoughtRow[]): StaleGraph {
  ...
  return {
    ...
    finishedBefore: stale.filter((r) => r.pool.recorded === "succeeded").length,
...
export type Sample = {
  ...
  calls: number;
  ms: number;
  /** The fresh parent vector against the stored one. NULL when the row had no vector to compare. */
...
export type CostModel = {
  ...
  meanMsPerRow: number;
  /** The mean without the two extreme ranks, since the sample gives each 1/n of the weight the corpus gives 1/rows. */
  ...
  editSeconds: number;
  /** Fresh against stored over the REUSED rows that had a vector. */
...
export function costModel(samples: readonly Sample[], corpus: { rows: number; chars: number; windowed: number }, edits: number): CostModel {
  ...
  const trimmedMs = sortedByChars.length > 2 ? sortedByChars.slice(1, -1).map((s) => s.ms) : ms;
  const trimmed = trimmedMs.length ? trimmedMs.reduce((a, b) => a + b, 0) / trimmedMs.length : NaN;
  const perK = chars > 0 ? sum / (chars / 1000) : NaN;
  const reused = samples.filter((s) => s.decision === "reuse" && s.cosineToStored !== null);
  const cos = reused.map((s) => s.cosineToStored as number);
...
export function verdict(s: Scenarios, cost: CostModel | null): Verdict {
  ...
  // are the edits A reused. Every edited id must be among B's recomputed, and
  ...
  else reasons.push(`B: ${s.edited.n} edits${s.edited.alreadyMissed ? ` (${s.edited.alreadyMissed} already a miss in A)` : ""} recompute exactly ${s.edited.n - s.edited.alreadyMissed} beyond the no-op's ${recomputed(a)}; ${b.reuse} reused`);

  const c = s.modelBump.tally;
  if (c.reuse !== 0) reasons.push(`NO-GO: a model bump reuses ${c.reuse} vectors labelled another model`);
  else reasons.push(`C: a model bump recomputes ${recomputed(c)}/${c.total}${cost ? `, ${fmtSeconds(cost.modelBumpSecondsByRows)} by rows (${fmtSeconds(cost.modelBumpSecondsTrimmed)} without the two extreme ranks) / ${fmtSeconds(cost.modelBumpSecondsByChars)} by characters at the measured rate` : " — cost not measured this run"}`);
  ...
  if (cost) {
    if (cost.minCosine === null) { provisional = true; reasons.push("provisional: no reused row with a stored vector was in the sample, so the reproducibility bar was not applied"); }
    ...
    else reasons.push(`the cached value reproduces: fresh against stored over ${cost.reusedCompared} reused rows, min cosine ${cost.minCosine.toFixed(4)} (1−cos ${fmtGap(cost.maxGap)}), median ${cost.medianCosine!.toFixed(4)}${cost.windowsCompared ? `; ${cost.windowsCompared} window vectors compared, min cosine ${cost.windowMinCosine!.toFixed(4)}` : "; no window vector compared"}`);
  } else {
    provisional = true;
...
/** 1 − cos in exponential form; `0` when the two are equal to the last bit. */
...
export function renderReport(o: Observation): string {
  ...
  L.push("");
  L.push(`corpus: ${c.thoughts} thoughts, ${fmtChars(c.chars)}; ${c.withVector} with a vector (${c.byModel.map((m) => `${m.model ?? "<unlabelled>"} ${m.n}`).join(", ") || "none"}); content_fingerprint = content_fingerprint_of(content) on ${c.fingerprintAgrees}/${c.thoughts}, NULL on ${c.fingerprintNull}; ${c.chunkedThoughts} windowed (${c.chunkRows} chunk rows); chunk_context ${c.config.chunk_context ?? "?"}; target ${o.scenarios.target}`);
  ...
  if (o.cost && o.samples) {
    ...
    L.push(`  per row: median ${(k.medianMsPerRow / 1000).toFixed(2)} s, mean ${(k.meanMsPerRow / 1000).toFixed(2)} s, mean without the two extreme ranks ${(k.meanMsPerRowTrimmed / 1000).toFixed(2)} s; ${(k.msPerKChar / 1000).toFixed(3)} s per 1k chars; ${k.fellBack} fell back to a head window`);
    ...
    L.push(`  the cached value against a fresh one, over the ${k.reusedCompared} reused rows: min cosine ${k.minCosine === null ? "—" : k.minCosine.toFixed(4)} (1−cos ${fmtGap(k.maxGap)}), median ${k.medianCosine === null ? "—" : k.medianCosine.toFixed(4)}; window vectors: ${k.windowsCompared} compared${k.windowMinCosine === null ? "" : `, min cosine ${k.windowMinCosine.toFixed(4)}`}`);
    L.push("  id                                    decision  chars  windows  calls     s   cosine   1−cos    windows-min");
    for (const x of o.samples) L.push(`  ${x.id}  ${x.decision.replace("recompute:", "").padEnd(8)}  ${String(x.chars).padStart(5)}  ${String(x.windows).padStart(7)}  ${String(x.calls).padStart(5)}  ${(x.ms / 1000).toFixed(2).padStart(5)}  ${x.cosineToStored === null ? "     —" : x.cosineToStored.toFixed(4)}   ${x.cosineToStored === null ? "—".padEnd(7) : fmtGap(1 - x.cosineToStored).padEnd(7)}  ${x.windowCosineMin === null ? "—" : `${x.windowCosineMin.toFixed(4)} over ${x.windowsCompared}`}${x.fellBack ? "  head window" : ""}`);
  } else {
  ...
  if (o.claims.length) {
    ...
    for (const g of graphCost(o.claims, c.thoughts)) L.push(`  ${g.key.padEnd(28)} n ${String(g.n).padStart(4)}  median ${g.medianS.toFixed(1).padStart(6)} s/row  → ${c.thoughts} thoughts ≈ ${g.rebuildHoursSequential.toFixed(1)} h`);
  }
  L.push("");
