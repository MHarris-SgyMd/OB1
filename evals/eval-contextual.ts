 ...
 */
async function providerCeiling(text: string): Promise<{ chars: number; truncates: boolean } | null> {
  // A provider that REFUSES over-length input rather than truncating it — hosted
  // APIs do — cannot be probed this way, and must not take the whole run down
  // with an unhandled rejection before a single arm has been scored.
  let full: number[];
  try {
    full = await embed(EMBED_MODEL, text);
  } catch (e) {
    console.error(`  (ceiling probe: ${EMBED_MODEL} refused ${text.length} chars — ${(e as Error).message.slice(0, 80)})`);
    return null;
  }
...
async function complete(prompt: string): Promise<string> {
  ...
  try {
    const r = await fetch(`${EVAL_BASE}/chat/completions`, {
      ...
      body: JSON.stringify({
        model: CTX_MODEL,
        // The server's contextualiseChunk sends exactly these two. A harness that
        // let a thinking model reason where production tells it not to would be
        // measuring different blurbs than the ones that get embedded — the drift
        // lib.ts exists to prevent, in the other half of the same capture path.
        temperature: 0,
...
/**
 * The cache key includes a digest of the PROMPT, not just the model and the
 * item.
 *
 * Without it, editing a template in db/config.mjs silently reuses answers
 * generated by the previous one, and the run reports numbers for a prompt it did
 * not use — the exact shape of failure this directory keeps finding, arriving
 * through the optimisation meant to make re-runs cheap. It nearly happened here:
 * `chunkTight` was added mid-investigation and only escaped because a new
 * template produced a new key rather than colliding with an old one.
 *
 * Cheap, non-cryptographic, and sufficient: the only thing being detected is
 * "this is not the text we asked last time".
 */
...
async function blurb(key: string, prompt: string, target: string): Promise<string> {
  const cacheKey = `${CTX_MODEL}::${key}::${digest(prompt)}`;
...
for (const d of CHUNKED) {
  for (let i = 0; i < d.windows.length; i++) {
    /**
     * Keyed by the prompt but deliberately NOT by CTX_MODEL. The query set is
     * the measuring instrument: it has to stay identical while the blurb model
     * varies, or a run on `embeddinggemma` would be scored against different
     * questions than a run on the default and the two tables could not be put
     * side by side. Editing the prompt does invalidate it, which is right.
     */
    const prompt = detailPrompt(d.title, d.windows[i]);
    const key = `detail::${d.id}::${i}::${digest(prompt)}`;
    if (!(key in cache)) {
      cache[key] = await complete(prompt);
      cacheDirty = true;
    }
    const q = cache[key];
...
console.log(
  ceiling === null
    ? `  provider ceiling: ${EMBED_MODEL} REFUSES a ${longest.text.length}-char input rather than truncating it, so the \`whole\` arms below are not measurable on this provider`
    : ceiling.truncates
