...
- **Every PR that touches `db/migrations/`, `server-portable/` or `evals/` ships a
  ...
  job's landing check (`scripts/check-landing.ts`) refuses a PR — and the merge
  queue refuses its group — whose change in those directories, test files
  (`test-*.ts`, `*.test.ts`) and Markdown aside, comes with no fragment; the
  refusal names the files that asked (SMD-1857).
