...
| `mxbai-embed-large` | 1024 | Measured here: 0.882 MRR on the 441-issue corpus, 10th of 11 and below `nomic-embed-text` (`evals/README.md`). The retired `recipes/local-ollama-embeddings` ranked it first of its three on four text pairs — the toy and the corpus disagree (SMD-2138). |
| `rjmalagon/gte-qwen2-1.5b-instruct-embed-f16` | 1536 | Unmeasured here; community-published, not an official Ollama model, and `db/config.mjs` knows no width for it, so a wrong `OB1_EMBEDDING_DIM` beside it is caught only when the first vector is refused. The retired recipe's only measurement was four text pairs — run `evals/` before choosing it (SMD-2138). |
