...
- Data imports: ChatGPT, Perplexity, X/Twitter, Instagram, Google Activity, Grok, Blogger/Journals, Gmail. An Obsidian or Markdown vault is the fork's `db/` tooling, not a recipe: `bun db/ingest-records.ts --url postgres://… --source markdown --markdown /path/to/vault --allow /path/to/vault`, then `bun db/reembed.ts --url postgres://…` with the embedding model declared as the server has it (`OB1_LLM_LOCAL=1` and the model variables, `SETUP.md`) (SMD-2137).
