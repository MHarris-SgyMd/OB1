...
- Data imports: ChatGPT, Perplexity, X/Twitter, Instagram, Google Activity, Grok, Blogger/Journals, Gmail. An Obsidian or Markdown vault is core, not a recipe: `bun db/ingest-records.ts --source markdown --markdown <vault> --allow <vault>`, then `bun db/reembed.ts` (SMD-2137).
