...
| `kubernetes-deployment` | keep + audited | Self-hosted K8s + own Postgres deployment path. Raw INSERTs audited under SMD-1524; guard owns `k8s/init.sql` / `openbrain.yml` / `index.ts` (OWN_DATABASE); CI `test-auth.ts:188` + `fork-checks.yml` deno-check. |
...
| `telegram-capture` | keep + audited → fold-in **SMD-1867** | Telegram quick-capture source; its README is a CI-driven write sample (`test-writes.ts:701-702`) and a guard fixture (check 10's write-sample exception in `scripts/check-fork-consistency.ts`). Capture adapter under SMD-1867. |
