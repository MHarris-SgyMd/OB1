...
> **On this fork (SMD-2126).** `import-readwise.py` opens supabase-py with a service-role key and captures through `rpc("upsert_thought")` — the right function over a transport this fork's stack does not run (SETUP.md), so the import stops at `create_client` (`--dry-run` runs). It becomes an adapter of the ingestion contract — the script emits items for `bun db/ingest-records.ts --items`, which writes them through `upsert_thought` — in SMD-2149 (after SMD-2136); the decision for the class is in `docs/vendored-disposition.md`.
