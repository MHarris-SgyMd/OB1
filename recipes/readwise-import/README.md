...
> **On this fork (SMD-2126).** `import-readwise.py` opens supabase-py with a service-role key and captures through `rpc("upsert_thought")` — the right function over a transport this fork's stack does not run (SETUP.md), so the import fails at its first request, `--dry-run` included. It becomes an adapter of the ingestion contract — the script emits items for `bun db/ingest-records.ts --source items --items`, which inserts each row itself with 003's fingerprint function, the audit actor and its `thought_sources` identity (`bun db/reembed.ts` embeds it) — in SMD-2149 (after SMD-2136); the decision for the class is in `docs/vendored-disposition.md`.
...
> **On this fork (FORK.md change 71, SMD-1524):** each highlight is stored through the database's 3-argument `upsert_thought`, one call per row, so it carries its content fingerprint, its vector's model label and the audit actor in the same statement; the batch insert this replaced left the fingerprint NULL (the row invisible to dedup until a backfill) and the label NULL, and needed bisection on `statement_timeout` and on a duplicate aborting the batch. A highlight whose text the brain already holds is not a unique violation but the function's `existed` — metadata merged, vector replaced, counted as already present. The `source_type`/`type` columns follow by one update per column per batch over the rows that lack that column — a fresh row, or one an interrupted earlier run left without them, which the id check cannot see and the function answers `existed`; a column already set, whichever writer set it, is left as it is. No audit actor is named: a script run by hand is what 008 keeps a NULL actor for. One function call per highlight is slower than one insert per 25: at PostgREST's typical 50–100 ms a call, budget ten to twenty minutes per 10K highlights. WIDTH: the vectors are `openai/text-embedding-3-small` at 1536, so the brain must be at that model and width (this fork's default is 1024, where every store fails).
