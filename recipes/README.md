...
| [Session Capture Hook](session-capture-hook/) | A session hook for Claude Code and Codex that captures one summary thought at each compaction and at session end, with the session's retrieved thoughts as provenance, through a capture-only key, after a secret scan; off until you paste the hooks it prints |
