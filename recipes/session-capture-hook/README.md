...
**A hit is blanked, not the session dropped** (SMD-2127). Each span the scan finds becomes `[redacted:<reason>]` — the value of an assignment, a flag or a URL's password (a quoted value to its closing quote, straight, curly, guillemet or backtick, spaces and all), so `LINEAR_API_KEY=[redacted:linear key, credential assignment, high-entropy token]` keeps the name; the whole of a prefixed key, a token, a private-key block, and with a hit of key material the base64 lines of the block around it, the whole line snapped when it is base64 alone — a wrapped key's line with no run of thirty-two, or a short final line, is no hit on its own — in the prompts, the outcome and the title before the text is rendered (so a key a clip would cut in half is blanked whole), then in the rendered text for what reaches it from elsewhere; three rules firing at one site are one marker naming the three. The text is scanned once more, sources included, and only a text that second scan finds clean is sent: one it does not — a URL whose blanked password leaves a digest in its path reading as a bare key, or a base64 line a name split off from its block and left beside a marker (the coarse backstop) — is **refused**, as every hit was before: the reason, the source and the character offset are printed, never the match; the exit code is 1; nothing is written. The session ends as it would have — exit 1 blocks nothing in either harness. The log's `captured` line carries `redactions=N`, the payload records each span's reason, offset and source, and `--dry-run` prints the text as it would be sent. `OB1_CAPTURE_ON_SECRET=refuse` in the hook's environment restores the refusal for every hit; `redact` is the default, and any other word is refused — a typo must not pick a mode: the hook exits 1 with nothing captured (and, as on any refused flag, posts nothing under `pending/` until the word is fixed; `--post` by hand still drains it), `--print-hook` and `--dry-run` exit 2 as on a bad flag, `--check` names it with exit 1. The marker is not yet on the thought's metadata — `capture_thought` takes none; that is SMD-2168's, once SMD-2014's `metadata` argument lands.
