...
- A summary that carries a secret is sent with the secret blanked to `[redacted:<reason>]`, the `captured` line saying `redactions=N`; one the blanking leaves a hit in — or any hit under `OB1_CAPTURE_ON_SECRET=refuse` — is refused with exit 1 and a `refused —` line naming the shape, the source and the offset; that episode is not sent, and the session's other episodes still are.
