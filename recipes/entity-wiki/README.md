...
> **On this fork (SMD-2126).** `generate-wiki.mjs` reaches the brain as a PostgREST client — `${SUPABASE_URL}/rest/v1/…` with a service-role key — and this fork's stack runs no PostgREST (SETUP.md), so the live mode fails at its first request (`--dry-run` runs). The port onto `compat/supabase-sql` under `bun` (`SUPABASE_URL` a `postgres://` string; the `node` commands below become `bun`) is SMD-2143; the decision for the class is in `docs/vendored-disposition.md`.
