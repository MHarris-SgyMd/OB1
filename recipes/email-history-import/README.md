...
**`--ingest-endpoint`** — POSTs to a capture server that handles embedding and metadata server-side (the Slack capture's shape, `integrations/slack-capture`). Requires `INGEST_URL` and `INGEST_KEY`. Use this if you run one.
...
7. **Upsert** into Supabase with SHA-256 [content fingerprint](../../recipes/content-fingerprint-dedup/README.md) — re-running produces zero duplicates
...
- `content_fingerprint`: Normalized SHA-256 hash for dedup (see [content fingerprint primitive](../../recipes/content-fingerprint-dedup/README.md))
