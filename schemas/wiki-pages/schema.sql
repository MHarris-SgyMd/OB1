...
--
-- gen_random_uuid() is a core function since Postgres 13. Upstream's file ran
-- `CREATE EXTENSION IF NOT EXISTS pgcrypto` here for it; this fork does not
-- (SMD-1796): the extension is not needed, and PGlite — where db/test-schema.ts
-- applies this file — does not ship it, so the statement stopped the file there.
-- ============================================================================
...
-- ─── RLS + grants — removed on this fork (SMD-1796) ────────────────────────
-- Upstream's file here ENABLEd ROW LEVEL SECURITY on the three tables, REVOKEd
-- them FROM PUBLIC, anon, authenticated and GRANTed them TO service_role (the
-- revision history SELECT, INSERT only — append-only, so the advertised
-- immutable history cannot be rewritten outside the RPCs; rows still go with
-- their section's ON DELETE CASCADE, which is a section deletion, not history
-- mutation). anon, authenticated and service_role are Supabase's roles: on
-- plain Postgres the REVOKE stops the file (`role "anon" does not exist`), and
-- RLS with no policy for the role you connect as denies it every row.
...
-- three tables (wiki_pages and wiki_sections SELECT, INSERT, UPDATE, DELETE;
