...
console.log("\n[34] Migration 034: query_log shape + CHECKs, the export join, and prune_query_log's bounded delete (SMD-1295)");
{
  ...
  // result_ids for the export join, plus 046's (SMD-1492) plain btree on
  ...
  assert(idx.some((r) => /USING btree \(logged_at\)/.test(r.indexdef)), "a plain btree on (logged_at) exists so prune's time-range DELETE range-scans (046, SMD-1492)");
