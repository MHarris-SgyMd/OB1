...
console.log("\n[8e] Migrations 037 and 038: the routing count is gated by a sample of the heap, drawn by TID range — the shape, the floor, and exactness with the gate reached");
{
  ...
  assert(lastDefinerOf("match_thoughts").startsWith("039"), `039 is the last definer of match_thoughts — 038's body with jit off (${lastDefinerOf("match_thoughts")})`);
  ...
  const file039 = files.find((f) => f.startsWith("039"))!;
  await db.exec(substituteMigration(readFileSync(join(MIGRATIONS, file039), "utf8"), migrationValues({ dim: EMBEDDING_DIM, model: EMBEDDING_MODEL, trgm: DEFAULT_TRGM_INDEX, backfillLimit: null, routeEstimateMinPages: 0 })));
  assert(/IF v_pages >= 0 THEN/.test(await shipped()), "039 applied with SchemaOptions.routeEstimateMinPages = 0: the sample runs on any heap");
...
console.log("\n[20] Migration 019: the row estimates and the plan setting — carried by 020, and the trap either re-apply springs");
{
  ...
  // Exactly three clauses — 014's scan mode, 019's plan setting, 039's jit
  ...
  // The candidate scan is 014's, byte for byte, through 019, 020, 037, 038
  // and 039: the three RETURN QUERY blocks' CTEs (direct, chunked, best) and the
  ...
  // collection; 039 added a SET clause and no statement; this holds "carried
  // verbatim" for the part that decides the plan.
  ...
  // function, 039 match_thoughts. A successor that redefines either fails
  // here on purpose, and the expectations move with the clauses it must
  // carry.
...
console.log("\n[21] Migration 020: the recency blend — identical at weight 0, and the formula above it");
{
  ...
  // The shipped body: 020's clauses as 019 handed them over, and 039's jit
  ...
  assert(Number(proc.prorows) === 10 && settings["enable_seqscan"] === "off" && settings["hnsw.iterative_scan"] === "relaxed_order" && settings["jit"] === "off" && Object.keys(settings).length === 3 && /ob1:filter-inside-scan/.test(proc.prosrc),
         "the shipped body carries what 019 handed 020: ROWS 10, its two settings and the sentinel — and 039's jit = off, exactly three settings");
