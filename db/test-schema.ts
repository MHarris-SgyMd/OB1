...
console.log("\n[34] Migration 034: query_log shape + CHECKs, the export join, and prune_query_log's bounded delete (SMD-1295)");
{
  ...
  // start — under PGlite a millisecond, so a row inserted a few statements
  // earlier can share the prune's now() and survive prune_query_log(0) (flaked
  // once, SMD-1498). Age the rows; the assumption was the flaw, not the function.
  ...
  // differs, and strictness decides only this equality — a concurrent writer's
  // row is safe by visibility and the window, not by the `<`.
  await db.transaction(async (tx) => {
    ...
    assert(sameTick === 0 && left === 1, "a row logged at the prune's own now() is kept — the bound is strict (logged_at < now()), as the COMMENT states");
    await tx.rollback();
  });
