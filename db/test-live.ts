...
console.log("\n[19] db/ingest-records.ts: the records upsert is source-labelled and idempotent, an edit moves one row, duplicate content is skipped (SMD-1806)");
{
  ...
  // 047's stamp marks every record with its name (the kind once the operator
...
console.log("\n[20] said_by on real pgvector: the mark 047 stamps is filtered through 014's route — 001's GIN index, scanned inside the call — and the answer is the operator's rows alone (SMD-1726)");
{
  ...
  // one call. Rows through two classified keys, stamped by 047's trigger as
  ...
  const load = async (key: string, n: number) => {
    const ids: string[] = [];
    for (let i = 0; i < n; i++) {
      const r = (await sql`SELECT upsert_thought(${`live 047 ${key} row ${i}`}, ${{ metadata: { source: "live" }, actor: { name: key, via: "test-live" } }}::jsonb, ${`[${unitVector(EMBEDDING_DIM).join(",")}]`}::vector) AS r`)[0].r as { id: string };
      ids.push(r.id);
    }
    return ids;
  };
  ...
  const kw = (await sql.unsafe(`SELECT id FROM search_thoughts_keyword('live 047', 200, 0, '{"actor_name": "bot-live"}'::jsonb)`)) as { id: string }[];
