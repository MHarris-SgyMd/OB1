-- =============================================================================
-- Migration 050: the actor on the row — who wrote a thought's current text,
...
--      keys are set to exactly that wherever the row differs — so a pre-050
...
--      reproduced 6 of 12). seq is exact for rows written after 050; rows from
...
COMMENT ON COLUMN thought_audit.seq IS
  'The order rows were written in — an identity, assigned at INSERT, exact for rows written after 050. created_at is now(), one value for every row a transaction writes, and id is a random uuid, so neither orders two writes to one thought inside a transaction; seq does. Rows from before 050 took theirs at the ALTER in heap order, which is not insertion order where 046''s backfill amended rows and VACUUM let later inserts fill the freed space — so read created_at first and seq as the tiebreak, as backfill_thought_actors does. Migration 050 / SMD-1726.';
