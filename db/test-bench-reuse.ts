 ...
 * SMD-1562, the exact pass's answers beside it: a reuse takes the first Q of
 * the queries the marker holds, computes only what it lacks, and extends the
 * marker. The claim that matters is that the answers a reuse takes from the
 * marker are the answers it would have computed — so this runs the bench
 * six times under one kept name at 150,000 rows (the smallest scale that is
 * kept: the before arm's ceiling is 100,000) and compares the tables that
 * depend on the oracle between a run that read the marker and a run that
 * computed, on the SAME kept index (two builds would give two HNSW graphs and
 * two recall figures, which would say nothing about the cache):
 *
...
const SCALE = 150_000;
/** One kept name per run of this suite, so a failed run's volume cannot be mistaken for the next run's. */
const KEEP = `test-reuse-${Date.now().toString(36)}`;

/** The shell's environment without its own OB1_* choices (a width or a query count from the shell would change what the bench measures), plus this run's. */
...
function scored(out: string): string {
  ...
  for (const line of out.split("\n")) {
    ...
    kept.push(`${section}: ${cells.filter((_, i) => !/\bms\b/.test(header![i] ?? "")).join(" | ")}`);
  }
  return kept.join("\n");
}

/** The section L cell for a reused corpus, without the build's timestamp. */
const sourceCell = (out: string) => /\| (reused \(built [^)]*\), oracle [^|]*) \|/.exec(out)?.[1].replace(/built [^)]*\)/, "built …)") ?? null;
...
try {
  console.log(`[1] build at ${SCALE.toLocaleString()} rows with Q=5, kept as ${KEEP}`);
  ...
  const remove = /Remove: (\S+) volume rm (\S+)/.exec(r1.out);
  assert(remove !== null, "the wrapper prints how to remove the kept volume");
  if (remove) [, runtime, volume] = remove;
  ...
  assert(sourceCell(r2.out) === "reused (built …), oracle reused", `section L says oracle reused (${sourceCell(r2.out)})`);
  assert(!r2.out.includes("marker extended"), "a marker holding more than this run asked is left as it is");
  ...
  assert(sourceCell(r3.out) === "reused (built …), oracle computed", `section L says oracle computed (${sourceCell(r3.out)})`);
  ...
  const scored2 = scored(r2.out);
  const scored3 = scored(r3.out);
  assert(scored2.split("\n").length >= 12, `the scored tables have rows to compare (${scored2.split("\n").length})`);
  assert(scored2 === scored3, "sections A, B, D and E from the marker's answers equal the ones from computed answers, on the same index");
  if (scored2 !== scored3) console.log(`--- from the marker\n${scored2}\n--- computed\n${scored3}`);
  ...
  assert(sourceCell(r4.out) === "reused (built …), oracle extended (3 of 6 from the marker)", `section L says oracle extended (${sourceCell(r4.out)})`);
  assert(r4.out.includes("marker extended: the exact pass's answers for 6 queries (had 3)"), "the marker now holds six");
  ...
  assert(sourceCell(r5.out) === "reused (built …), oracle reused", `section L says oracle reused (${sourceCell(r5.out)})`);
  assert(scored(r4.out) === scored(r5.out), "sections A, B, D and E from the extended marker equal the run that extended it");
  if (scored(r4.out) !== scored(r5.out)) console.log(`--- extended\n${scored(r4.out)}\n--- from the marker\n${scored(r5.out)}`);
  ...
  if (runtime && volume) {
    const rm = await runScript([runtime, "volume", "rm", volume], { cwd: HERE });
    assert(rm.code === 0, `the kept volume ${volume} is removed (exit ${rm.code})`);
  } else {
    console.error(`  the kept volume could not be identified from the wrapper's output; look for ob1-pg-keep-${KEEP}`);
  }
}
