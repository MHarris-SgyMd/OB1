...
export const ROLE_GRANTS = Object.freeze({
  ...
  // it raising — since 054 only on a stale key's lookup, as a fresh one writes
  // nothing, so a missing UPDATE shows up minutes later, not at once. A capture tolerates all of this (SELECT on ob1_agents excepted,
