...
/** Every succeeded claim row under a key naming a model, with model, own_key, accepted and its timestamps — what 030 reads, and LATEST_UNACCEPTED_CLAIM_SQL is built on. Substituted into 030: changing it is a data migration. Needs 015. */
export const CLAIM_EVIDENCE_ROWS_SQL: string;
/** Per thought, the latest succeeded row that is not an acceptance (thought_id, model, finished_at) — 030's second statement's choice, byte for byte; migrate.ts's bracket around 021 relabels by it (SMD-1421). */
export const LATEST_UNACCEPTED_CLAIM_SQL: string;
