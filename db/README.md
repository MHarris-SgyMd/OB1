...
`bun test-schema.ts` prints `976 assertions: 976 passed, 0 failed` and `PASS`.
...
./with-postgres.sh bun test-live.ts         # 543 assertions, real server, throwaway container (fewer, as one skipped group, on PostgreSQL 18 or without JIT)
