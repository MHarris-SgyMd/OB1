...
`bun test-schema.ts` prints `1018 assertions: 1018 passed, 0 failed` and `PASS`.
Against a real database, `bun migrate.ts` reports forty-two migrations applied, and
...
```bash
bun test-schema.ts                          # 1018 assertions, PGlite, no container
