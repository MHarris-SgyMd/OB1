...
`bun test-schema.ts` prints `502 assertions: 502 passed, 0 failed` and `PASS`.
...
this list marks the row holding the key, so what 023 decided is readable here.
Stop a pass before applying it: the workers would wait on that lock and their
leases expire. After it a row without a fingerprint is a twin, or
...
```bash
bun test-schema.ts                    # 502 assertions, PGlite, no container
...
asserts 502 properties, including:
