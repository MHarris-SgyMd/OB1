...
// SMD-1726: who wrote a thought's current text, on the read path. Migration 048
 ...
 * are the keys migration 048 stamps, so the store, the query log and the plan
...
export function withActorFilter(filter: Record<string, unknown>, saidBy: string | undefined, actor: string | undefined): Record<string, unknown> {
  const out = { ...filter };
  // The stamp trims the key's name (048), so the argument is trimmed here too —
 ...
 * migration 048 stamps. Absent when the row carries neither (a write from
      ...
      try {
        ...
        const results = data.map(
          (t, i) => {
            ...
            // SMD-1726: who wrote the current text, from the key (048); its own
  ...
  server.registerTool(
    "list_thoughts",
    {
      ...
      inputSchema: {
        ...
        // SMD-1726: who wrote it — the two keys 048 stamps, as containment
