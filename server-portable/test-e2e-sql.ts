...
// is the key's name, and migration 049 stamps who wrote a thought from it.
...
console.log("\n[10c] Who wrote it, over MCP: a hit says By: <key> (kind) from the key that made the write, said_by and actor filter on it through the one search operation, a client's metadata cannot set it, and the folded filter reaches the log (SMD-1726, migration 049)");
{
  const sql = new SQL({ url: URL_, max: 1 });
  ...
  // it in the envelope, and 049's trigger stamps the row from the registry.
  ...
  // agent's key patching the operator's row to claim it changes nothing (049:
